...
population |>
  ...
  # add labels
  labs(
    title = "States's population (2020 and 2021)",
    x = "State",
    y = "Population"
  )
```
...
**Answer:** No, not all the data is there; we only see 1,000 rows. This is because the CDC has a default limit, which prevents us from retrieving more rows.
...
```{r}
# Your code here
deaths |> # already filter out the "United States" and "New York City" in last question
  group_by(state) |> 
